...
async function testHighLevelV2() {
  ...
  console.log(
    `- API Key: ${apiKey ? `${colors.green}✓ Found (${apiKey.substring(0, 20)}...)${colors.reset}` : `${colors.red}✗ Not found${colors.reset}`}`
  );
  console.log(
  ...
  if (!apiKey || !locationId) {
    ...
    console.log(
      `${colors.yellow}Please ensure GOHIGHLEVEL_API_KEY and GOHIGHLEVEL_LOCATION_ID are set${colors.reset}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (parts.length === 3) {
      ...
      console.log(
        `- Token Type: ${payload.company_id ? 'Company/Agency' : payload.location_id ? 'Location' : 'Unknown'}`
      );
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        `${colors.green}✓ Successfully connected to GoHighLevel v2!${colors.reset}`
      );
      ...
      if (response.status === 401) {
        console.log(
          `${colors.yellow}Hint: The API key may be invalid or expired${colors.reset}`
        );
      }
    }
  } catch (e) {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      console.log(
        `${colors.green}✓ Contact lookup endpoint works!${colors.reset}`
      );
      const contacts = data.contacts || [];
      console.log(
        `  Found ${contacts.length} contact(s) with email: ${testEmail}`
      );
      testsPassed++;
    } else if (response.status === 404) {
      console.log(
        `${colors.green}✓ Contact lookup works (no contact found - expected)${colors.reset}`
      );
      testsPassed++;
    } else {
      const errorText = await response.text();
    ...
    if (response.ok) {
      ...
      if (contactId) {
        ...
        if (deleteResponse.ok) {
          ...
          console.log(
            `  ${colors.yellow}⚠ Could not delete test contact${colors.reset}`
          );
        }
      }
    } else {
    ...
    if (response.ok) {
      ...
      if (fields.length > 0) {
        ...
        fields.slice(0, 3).forEach(field => {
          console.log(
            `    - ${field.name || field.fieldKey} (${field.dataType || field.type || 'text'})`
          );
        });
      }
      testsPassed++;
      ...
      console.log(
        `${colors.yellow}⚠ Could not fetch custom fields (${response.status})${colors.reset}`
      );
      testsFailed++;
    }
  } catch (e) {
  ...
  if (testsPassed > 0 && testsFailed === 0) {
    console.log(
      `${colors.green}✓ All tests passed! (${testsPassed}/${testsPassed + testsFailed})${colors.reset}`
    );
    console.log(
      `${colors.green}✓ GoHighLevel API v2 connection is working!${colors.reset}`
    );
    console.log(`${colors.green}✓ Integration is ready to use!${colors.reset}`);
  } else if (testsPassed > 0) {
    console.log(
      `${colors.yellow}⚠ Partial success: ${testsPassed} passed, ${testsFailed} failed${colors.reset}`
    );
    console.log(
      `${colors.yellow}Some API features are working, but there may be issues${colors.reset}`
    );
  } else {
    console.log(
      `${colors.red}✗ All tests failed (${testsFailed} tests)${colors.reset}`
    );
    console.log(
      `${colors.red}API connection is not working properly${colors.reset}`
    );

    console.log(`\n${colors.yellow}Troubleshooting:${colors.reset}`);
    console.log('1. Verify the API key is valid and not expired');
    console.log(
      '2. Check if this is the correct type of API key (Company vs Location)'
    );
