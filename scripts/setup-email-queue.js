...
async function setupEmailQueue() {
  ...
  try {
    // Read the SQL file
    const sql = fs.readFileSync(path.join(__dirname, 'create-email-queue.sql'), 'utf8');
    ...
    if (error) {
      ...
      console.log('RPC method not available, executing statements individually...');
      ...
      for (const statement of statements) {
        ...
        if (!response.ok) {
          console.warn('Statement might have failed:', statement.substring(0, 50));
    ...
    if (!testError) {
      ...
      console.log('⚠️  Could not confirm email_queue table. You may need to run the SQL manually in Supabase dashboard.');
    }
