...
async function testHighLevelConnection() {
  ...
  console.log(`${colors.cyan}   GoHighLevel API Connection Test${colors.reset}`);
  console.log(`${colors.cyan}======================================${colors.reset}\n`);
  ...
  console.log(`- API Key: ${apiKey ? `${colors.green}✓ Found (${apiKey.substring(0, 10)}...)${colors.reset}` : `${colors.red}✗ Not found${colors.reset}`}`);
  ...
  if (!apiKey || !locationId) {
    ...
    console.log(`${colors.yellow}Please ensure GHL_API_KEY and GHL_LOCATION_ID are set in .env.production${colors.reset}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`${colors.cyan}Test 1: Fetching Location Details${colors.reset}`);
    ...
    if (locationResponse.ok) {
      ...
      console.log(`  Location Name: ${locationData.name || locationData.companyName || 'N/A'}`);
      ...
      console.error(`  Status: ${locationResponse.status} ${locationResponse.statusText}`);
      console.error(`  Response: ${errorText}\n`);
      
      if (locationResponse.status === 401) {
        console.log(`${colors.yellow}Hint: Check if your API key is valid and has the correct permissions${colors.reset}`);
      }
      if (locationResponse.status === 404) {
        console.log(`${colors.yellow}Hint: Check if the Location ID is correct${colors.reset}`);
      }
    }
    ...
    if (contactResponse.ok) {
      ...
      console.log(`  Contact ID: ${contactData.contact?.id || contactData.id || 'N/A'}`);
      ...
      if (contactData.contact?.id || contactData.id) {
        ...
        if (deleteResponse.ok) {
          console.log(`${colors.green}✓ Test contact deleted successfully${colors.reset}\n`);
        } else {
          console.log(`${colors.yellow}⚠ Could not delete test contact (manual cleanup may be needed)${colors.reset}\n`);
        }
      }
    } else {
      const errorText = await contactResponse.text();
      console.error(`${colors.red}✗ Failed to create test contact${colors.reset}`);
      console.error(`  Status: ${contactResponse.status} ${contactResponse.statusText}`);
    ...
    if (customFieldsResponse.ok) {
      ...
      if (fields.length > 0) {
        ...
        fields.slice(0, 3).forEach(field => {
          console.log(`    - ${field.name || field.fieldKey} (${field.dataType || 'text'})`);
        });
      }
      console.log();
    } else {
      console.log(`${colors.yellow}⚠ Could not fetch custom fields${colors.reset}\n`);
    }
    ...
    console.log(`${colors.cyan}======================================${colors.reset}`);
    console.log(`${colors.green}✓ GoHighLevel API connection is working!${colors.reset}`);
