...
async function debugTicketCreation() {
  ...
  try {
    ...
    if (selectError) {
      console.error('❌ Cannot SELECT from support_tickets:', selectError.message);
    } else {
    ...
    if (insertError) {
      console.error('❌ Cannot INSERT into support_tickets:', insertError.message);
      ...
      if (insertError2) {
        ...
        console.log('✅ Created ticket with explicit number:', newTicket2.ticket_number);
    ...
    console.log('   To verify RLS is enabled, run this in Supabase SQL Editor:');
    console.log('   SELECT tablename, rowsecurity FROM pg_tables WHERE tablename = \'support_tickets\';');
    console.log('\n   To see all policies:');
    console.log('   SELECT * FROM pg_policies WHERE tablename = \'support_tickets\';');
