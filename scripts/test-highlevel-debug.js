...
async function debugHighLevelConnection() {
  ...
  try {
    ...
    if (parts.length === 3) {
      ...
      if (payload.exp) {
        ...
        if (payload.exp < now) {
          console.log(`${colors.red}⚠ Token appears to be expired!${colors.reset}`);
        }
      }
      
      // Check if this is a company/agency token or location token
      if (payload.company_id) {
        console.log(`${colors.yellow}This appears to be a Company/Agency level token${colors.reset}`);
        console.log(`Company ID: ${payload.company_id}`);
      }
      if (payload.location_id) {
        console.log(`${colors.yellow}This appears to be a Location level token${colors.reset}`);
        console.log(`Token Location ID: ${payload.location_id}`);
        if (payload.location_id !== locationId) {
          console.log(`${colors.red}⚠ Token location ID doesn't match configured location ID!${colors.reset}`);
        }
      }
    }
  } catch (e) {
    ...
    try {
      ...
      for (const method of authMethods) {
        ...
        const response = await fetch(`${baseUrl}/locations/${locationId}`, {
          ...
          timeout: 5000
        }).catch(err => ({ ok: false, status: 'Network Error', statusText: err.message }));

        if (response.ok) {
          console.log(`${colors.green}✓ SUCCESS!${colors.reset}`);
          const data = await response.json();
          console.log(`    Location Name: ${data.name || data.companyName || 'N/A'}`);
          console.log(`    ${colors.green}This endpoint and auth method work!${colors.reset}`);
  ...
  console.log('- GoHighLevel API Docs: https://highlevel.stoplight.io/docs/integrations/');
