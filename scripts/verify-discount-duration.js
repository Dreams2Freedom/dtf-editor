...
async function verifyDiscountDuration() {
  ...
  try {
    ...
    console.log('Regular monthly price: $' + (subscription.items.data[0].price.unit_amount / 100));
    console.log('Billing cycle: ' + subscription.items.data[0].price.recurring.interval);
    ...
    if (subscription.discount) {
      ...
      console.log('Duration in months: ' + (discount.coupon.duration_in_months || 'N/A'));
      ...
      if (discount.coupon.duration === 'once') {
        ...
        console.log('After that, price returns to regular $' + (subscription.items.data[0].price.unit_amount / 100));
      }
      
      // Show when discount ends
      if (discount.end) {
        console.log('Discount expires: ' + new Date(discount.end * 1000).toLocaleDateString());
      } else {
    ...
    console.log('   Amount: $' + discountedPrice.toFixed(2) + ' (50% OFF - ONE TIME ONLY)');
    
    console.log('\n2. Following billing (' + followingBillingDate.toLocaleDateString() + '):');
    console.log('   Amount: $' + regularPrice.toFixed(2) + ' (BACK TO REGULAR PRICE)');
    ...
    console.log('- duration: "' + subscription.discount.coupon.duration + '" (means applies once)');
    console.log('- times_redeemed: ' + subscription.discount.coupon.times_redeemed);
    console.log('- max_redemptions: ' + subscription.discount.coupon.max_redemptions);
