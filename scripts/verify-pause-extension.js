...
async function verifyPauseExtension() {
  ...
  if (subscription.pause_collection) {
    ...
    const extensionDays = Math.round(
      (resumeDate - currentEnd) / (1000 * 60 * 60 * 24)
    );
    ...
    console.log(
      ...
      user.subscription_paused_until
        ? new Date(user.subscription_paused_until).toLocaleDateString()
        : 'Not set'
    );
    ...
    if (user.subscription_paused_until) {
      ...
      const dateDiff =
        Math.abs(dbResumeDate - resumeDate) / (1000 * 60 * 60 * 24);
      if (dateDiff < 1) {
        console.log('✅ Database and Stripe dates match');
      } else {
        console.log(
          '⚠️  Database and Stripe dates differ by',
          dateDiff,
          'days'
        );
      }
  ...
  if (events && events.length > 0) {
    ...
    console.log(
      'Event Resume Date:',
      new Date(events[0].event_data.resume_date).toLocaleDateString()
    );
  }
}
