...
async function testBulkCreditsAPI() {
  try {
    ...
    const usersResponse = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/profiles?subscription_plan=eq.free&is_admin=eq.false&select=id,email,credits_remaining&limit=2`, {
      headers: {
        'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
    ...
    const response = await fetch('http://localhost:3000/api/admin/users/bulk-credits', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const checkResponse = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/profiles?id=in.(${userIds.join(',')})&select=id,email,credits_remaining`, {
      headers: {
        'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
        'Authorization': `Bearer ${authData.access_token}`
      }
...
console.log('Note: Update the admin email and password in the script before running');
