-- Ensure RPC functions exist for managing processed images
-- These functions bypass RLS issues and are used by the application

-- Drop existing functions if they exist to recreate them
DROP FUNCTION IF EXISTS insert_processed_image CASCADE;
DROP FUNCTION IF EXISTS get_user_images CASCADE;
DROP FUNCTION IF EXISTS delete_processed_image CASCADE;
...
-- Function to get user images (returns all columns including expires_at)
...
GRANT EXECUTE ON FUNCTION get_user_images TO service_role;
GRANT EXECUTE ON FUNCTION delete_processed_image TO service_role;
...
FROM pg_proc 
WHERE proname IN ('insert_processed_image', 'get_user_images', 'delete_processed_image')
ORDER BY proname;