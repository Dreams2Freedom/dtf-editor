      ...
      if (user) {
        ...
        if (uploads && uploads.length > 0) {
          ...
          uploads.forEach(upload => {
            console.log(
              `  - ${upload.filename} (${(upload.file_size / 1024).toFixed(2)} KB) - ${new Date(upload.created_at).toLocaleString()}`
            );
          });
        } else {
          console.log('\n📁 No uploads found');
        ...
        if (payments && payments.length > 0) {
          ...
          payments.forEach(payment => {
            console.log(
              `  - $${(payment.amount / 100).toFixed(2)} - ${payment.status} - ${new Date(payment.created_at).toLocaleString()}`
            );
          });
        } else {
          console.log('\n💳 No payments found');
