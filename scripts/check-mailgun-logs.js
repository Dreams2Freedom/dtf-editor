  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      ...
      data.items.forEach((event, index) => {
        ...
        const recipient = event.recipient || event.message?.recipients || 'Unknown';
        ...
        if (event.delivery_status) {
          console.log(`   Status: ${event.delivery_status.description || event.delivery_status.message}`);
        }
      ...
      const failed = data.items.filter(e => e.event === 'failed' || e.event === 'bounced').length;
    ...
    if (response.ok) {
      ...
      if (data.stats && data.stats.length > 0) {
        ...
        if (stats.delivered?.total > 0) {
          const deliveryRate = ((stats.delivered.total / stats.accepted.total) * 100).toFixed(1);
