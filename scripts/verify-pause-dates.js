...
async function verifyPauseDates() {
  ...
  console.log('Resume Date (from event):', new Date(eventData.resume_date).toLocaleString());
  ...
  if (user.stripe_subscription_id) {
    try {
      ...
      console.log('\n=== STRIPE SUBSCRIPTION ===');
      console.log('Current Period Start:', new Date(subscription.current_period_start * 1000).toLocaleString());
      ...
      if (subscription.pause_collection) {
        ...
        const pauseDays = Math.round((resumeDate - currentPeriodEnd) / (1000 * 60 * 60 * 24));
        ...
        console.log('Expected days for', eventData.duration + ':', expectedDays);
        ...
        const daysSincePause = Math.round((resumeDate - pauseDate) / (1000 * 60 * 60 * 24));
        
        console.log('\n=== ALTERNATIVE CALCULATION ===');
        console.log('Days from pause date to resume:', daysSincePause);
        console.log('This suggests the pause was applied from a date around:', 
          new Date(resumeDate.getTime() - expectedDays * 24 * 60 * 60 * 1000).toLocaleDateString());
        
        if (Math.abs(pauseDays - expectedDays) > 2) {
          console.log('\n⚠️  WARNING: Pause duration may not match expected value');
          console.log('This could be because the subscription period end changed after the pause was applied');
        } else {
  ...
  console.log('\n=== DATABASE VS STRIPE ===');
  console.log('DB Resume Date:', user.subscription_paused_until ? new Date(user.subscription_paused_until).toLocaleString() : 'Not set');
