...
async function applyMigration() {
  try {
    ...
    const migrationPath = path.join(
      ...
      'supabase',
      'migrations',
      '011_add_user_settings_columns.sql'
    );
    ...
    if (error) {
      ...
      console.log(
        '⚠️  exec_sql not available, executing statements individually...\n'
      );
      ...
      for (const statement of statements) {
        ...
        console.log(
          '⚠️  Note: This statement needs to be run directly in Supabase dashboard'
        );
      }
    ...
    if (!verifyError && columns && columns.length > 0) {
      ...
      console.log(
        ...
        'notification_preferences' in profile ? '✓' : '✗'
      );
