...
async function testCompanyAPI() {
  ...
  try {
    ...
    if (companyResponse.ok) {
      ...
      console.log(`  Company data:`, JSON.stringify(data, null, 2).substring(0, 200));
    } else {
      const error = await companyResponse.text();
      console.log(`${colors.red}✗ Company API failed: ${companyResponse.status}${colors.reset}`);
      console.log(`  Error:`, error.substring(0, 200));
    }
  } catch (e) {
    console.log(`${colors.red}✗ Company API error: ${e.message}${colors.reset}`);
  }
    ...
    if (locationsResponse.ok) {
      ...
      if (locations.length > 0) {
        locations.slice(0, 3).forEach(loc => {
          console.log(`  - ${loc.name || loc.companyName} (ID: ${loc.id || loc._id})`);
        });
      }
    } else {
      const error = await locationsResponse.text();
      console.log(`${colors.red}✗ Locations API failed: ${locationsResponse.status}${colors.reset}`);
      console.log(`  Error:`, error.substring(0, 200));
    }
  } catch (e) {
    console.log(`${colors.red}✗ Locations API error: ${e.message}${colors.reset}`);
  }
  ...
  try {
    ...
    if (oauthResponse.ok) {
      ...
      if (data.access_token) {
        ...
        const locationTokenTest = await fetch(`${baseUrl}/locations/${locationId}`, {
          headers: {
        ...
        if (locationTokenTest.ok) {
          ...
          console.log(`${colors.green}  SUCCESS! Working Configuration Found${colors.reset}`);
      ...
      console.log(`${colors.red}✗ OAuth endpoint failed: ${oauthResponse.status}${colors.reset}`);
      console.log(`  Error:`, error.substring(0, 200));
    }
  } catch (e) {
  ...
  try {
    ...
    if (usersResponse.ok) {
      ...
      console.log(`${colors.red}✗ Users API failed: ${usersResponse.status}${colors.reset}`);
    }
  } catch (e) {
