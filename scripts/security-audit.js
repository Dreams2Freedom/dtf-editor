...
function categorizeEndpoint(filePath) {
  ...
  if (filePath.includes('/storage/') || filePath.includes('/upload')) return 'storage';
...
function analyzeEndpoint(filePath, content) {
  ...
  const publicEndpoints = ['/api/webhooks/', '/api/auth/callback', '/api/test-', '/api/debug-', '/pricing', '/coming-soon'];
  const requiresAuth = !publicEndpoints.some(p => endpoint.includes(p));
  
  if (requiresAuth) {
    const hasAuth = SECURITY_CHECKS.authentication.patterns.some(pattern => pattern.test(content));
    if (!hasAuth) {
  ...
  if (category === 'admin') {
    const hasAdminCheck = SECURITY_CHECKS.adminCheck.patterns.some(pattern => pattern.test(content));
    if (!hasAdminCheck) {
  ...
  if (methods.some(m => ['POST', 'PUT', 'PATCH'].includes(m))) {
    const hasValidation = SECURITY_CHECKS.inputValidation.patterns.some(pattern => pattern.test(content));
    if (!hasValidation) {
  ...
  const hasErrorHandling = SECURITY_CHECKS.errorHandling.patterns.some(pattern => pattern.test(content));
  if (!hasErrorHandling) {
  ...
  // Check rate limiting
  const hasRateLimiting = SECURITY_CHECKS.rateLimiting.patterns.some(pattern => pattern.test(content));
  ...
  const hasSQLVuln = SECURITY_CHECKS.sqlInjection.patterns.some(pattern => pattern.test(content)) &&
                     !SECURITY_CHECKS.sqlInjection.antiPatterns.some(pattern => pattern.test(content));
  if (hasSQLVuln) {
    issues.push('❌ Potential SQL injection vulnerability');
  }
  
  // Check for hardcoded secrets
  const hasSecrets = SECURITY_CHECKS.secrets.patterns.some(pattern => pattern.test(content)) &&
                     !SECURITY_CHECKS.secrets.antiPatterns.some(pattern => pattern.test(content));
  if (hasSecrets) {
    const lines = content.split('\n');
    lines.forEach((line, idx) => {
      if (SECURITY_CHECKS.secrets.patterns.some(p => p.test(line)) && 
  ...
  if (['admin', 'payment', 'auth'].includes(category)) {
    const hasLogging = SECURITY_CHECKS.logging.patterns.some(pattern => pattern.test(content));
    if (!hasLogging) {
  ...
  // Check CORS headers
  const hasCORS = SECURITY_CHECKS.cors.patterns.some(pattern => pattern.test(content));
  if (hasCORS) {
  ...
  return {
    ...
    implemented,
    requiresAuth,
    severity: issues.length > 0 ? 'high' : warnings.length > 2 ? 'medium' : 'low'
  };
}
...
function performAudit() {
  ...
  routes.forEach(routePath => {
    ...
    if (analysis.implemented.includes('✅ Authentication')) results.summary.authentication++;
    if (analysis.implemented.includes('✅ Admin authorization')) results.summary.adminAuth++;
    if (analysis.implemented.includes('✅ Input validation')) results.summary.inputValidation++;
    if (analysis.implemented.includes('✅ Error handling')) results.summary.errorHandling++;
    if (analysis.implemented.includes('✅ Rate limiting')) results.summary.rateLimiting++;
  ...
  console.log(`✅ Secure: ${results.secure} (${((results.secure/results.total)*100).toFixed(1)}%)`);
  console.log(`⚠️  Warnings: ${results.warnings} (${((results.warnings/results.total)*100).toFixed(1)}%)`);
  console.log(`❌ Critical Issues: ${results.critical} (${((results.critical/results.total)*100).toFixed(1)}%)`);
  
  console.log('\n📈 SECURITY IMPLEMENTATION COVERAGE\n');
  console.log(`Authentication: ${results.summary.authentication}/${results.total} (${((results.summary.authentication/results.total)*100).toFixed(1)}%)`);
  console.log(`Admin Auth: ${results.summary.adminAuth}/${results.byCategory.admin?.total || 0}`);
  ...
  Object.entries(results.byCategory).forEach(([category, data]) => {
    console.log(`${category.toUpperCase()}: ${data.total} endpoints (${data.secure} secure)`);
    if (data.issues.length > 0) {
      console.log(`  Issues in: ${data.issues.slice(0, 3).join(', ')}${data.issues.length > 3 ? '...' : ''}`);
    }
  });
