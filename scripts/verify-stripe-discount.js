...
async function verifyDiscount() {
  ...
  try {
    ...
    console.log(
      'Regular price: $' + subscription.items.data[0].price.unit_amount / 100
    );

    if (subscription.discount) {
      const regularAmount = subscription.items.data[0].price.unit_amount / 100;
      const discountAmount =
        regularAmount * (subscription.discount.coupon.percent_off / 100);
    ...
    invoices.data.forEach((invoice, index) => {
      ...
      if (invoice.discount) {
        console.log(
          'Had discount: Yes (' + invoice.discount.coupon.percent_off + '% off)'
        );
      }
    });
  } catch (error) {
