...
async function checkShannonImages() {
  ...
  try {
    ...
    const { data: rpcImages, error: rpcError } = await supabase.rpc(
      'get_user_images',
      {
        p_user_id: shannonId,
      }
    ...
    const { data: herodImages, error: herodError } = await supabase.rpc(
      'get_user_images',
      {
    ...
    if (herodError) {
      ...
      console.log(
        '✅ Found',
        herodImages?.length || 0,
        'images for shannonherod@gmail.com'
      );
    }
