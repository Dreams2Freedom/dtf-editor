...
async function addExample() {
  ...
  // Select service
  const services = ['Deep-Image.ai', 'ClippingMagic', 'Vectorizer.ai', 'OpenAI', 'Stripe', 'Supabase'];
  ...
  // Get example details
  const title = await question('Example title (e.g., "Basic Upscaling Example"): ');
...
`;
  ...
  const sectionRegex = new RegExp(`## ${service.replace('.', '\\.')}([\\s\\S]*?)(?=\\n---\\n|$)`);
