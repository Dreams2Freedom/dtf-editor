...
adminRoutes.forEach(file => {
  ...
  const hasAuditLog =
    content.includes('logAdminAction') || content.includes('adminAudit');
  ...
  if (hasAuditLog) {
    ...
    const needsLogging =
      ...
      (content.includes('export async function') ||
        content.includes('export function')); // Has route handlers
...
console.log(
  `   Coverage: ${Math.round((routesWithAuditLog.length / adminRoutes.length) * 100)}%`
);
