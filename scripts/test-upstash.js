...
async function testUpstash() {
  ...
  if (!url || !token) {
    ...
    console.log('2. Create a new Redis database (choose region close to your app)');
  ...
  try {
    ...
    await redis.setex(testKey, 60, JSON.stringify({ count: 1, resetAt: Date.now() + 60000 }));
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      console.log(`Request ${i}: ${success ? '✅ Allowed' : '❌ Blocked'} (${remaining}/${limit} remaining)`);
      
      if (!success && i === 6) {
        console.log(`   Rate limit exceeded! Reset at: ${new Date(reset).toLocaleTimeString()}`);
      }
    }
