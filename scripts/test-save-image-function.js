...
async function testSaveImage() {
  ...
  try {
    ...
    const { data: imageId, error: insertError } = await supabase.rpc('insert_processed_image', {
      p_user_id: shannonId,
    ...
    if (insertError) {
      ...
      const { data: images, error: getError } = await supabase.rpc('get_user_images', {
        p_user_id: shannonId
      });
      ...
      if (getError) {
        ...
        if (testImage) {
          ...
          console.log('   Expires at:', testImage.expires_at || 'Never (permanent storage)');
        }
      }
      ...
      if (imageId) {
        ...
        const { data: deleted, error: deleteError } = await supabase.rpc('delete_processed_image', {
          p_image_id: imageId,
          p_user_id: shannonId
        });
