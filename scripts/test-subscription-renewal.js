  ...
  try {
    ...
    if (updatedUser.credits_remaining === plan.credits) {
      ...
      logTest(
        'Initial credits added',
        'fail',
        `Expected: ${plan.credits}, Got: ${updatedUser.credits_remaining}`
      );
    }
  ...
  try {
    ...
    if (creditsAdded === plan.credits) {
      ...
      logTest(
        'Renewal credits added',
        'fail',
        `Expected: ${plan.credits}, Added: ${creditsAdded}`
      );
    }
  ...
  try {
    ...
    if (expiringCredits && expiringCredits.length > 0) {
      const daysUntilExpiry = Math.floor(
        (new Date(expiringCredits[0].expires_at) - new Date()) /
          (1000 * 60 * 60 * 24)
      );
      logTest(
        'Credit expiration tracked',
        'pass',
...
async function testSubscriptionUpgrade(supabase, user) {
  ...
  try {
    ...
    log(
      `Current plan: ${before.subscription_status}, Credits: ${before.credits_remaining}`,
      colors.cyan
    );

    // Simulate upgrade
    const proratedCredits = Math.floor(
      (newPlan.credits - oldPlan.credits) * 0.5
    ); // 50% proration
    ...
    if (after.subscription_status === 'starter') {
      logTest(
        ...
        'pass',
        `New plan: ${after.subscription_status}`
      );
    } else {
      logTest(
        'Subscription upgraded',
    ...
    if (
      ...
      before.credits_remaining + proratedCredits
    ) {
      logTest(
        'Prorated credits added',
        'pass',
        `Added: ${proratedCredits}, Total: ${after.credits_remaining}`
      );
    } else {
      logTest(
        'Prorated credits added',
        'fail',
        `Expected: ${before.credits_remaining + proratedCredits}, Got: ${after.credits_remaining}`
      );
    }
  ...
  try {
    ...
    if (userData.subscription_status === 'past_due') {
      logTest(
        ...
        'pass',
        'Payment failed status recorded'
      );
    } else {
      logTest(
        'Status set to past_due',
    ...
    logTest(
      'Payment retry handled',
      'pass',
      `Credits added after successful retry`
    );
...
async function runTests() {
  ...
  try {
    ...
    log(
      `Tests Passed: ${testsPassed}/${totalTests} (${passRate}%)`,
      resultColor
    );

    if (testsPassed === totalTests) {
      log(
        '\n🎉 All tests passed! Subscription renewal credits are working correctly.',
        colors.green
      );
    } else {
