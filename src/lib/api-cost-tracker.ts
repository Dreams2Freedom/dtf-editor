...
export class ApiCostTracker {
   ...
   */
  private static async getDynamicCost(
    ...
    try {
      ...
      console.warn(
        `[ApiCostTracker] Failed to fetch dynamic cost for ${provider}/${operation}, using fallback`
      );
    }
  ...
  static async logUsage(params: {
    ...
    try {
      ...
      if (params.provider === 'stripe' && params.stripeAmount) {
        ...
        const stripePercentage = await this.getDynamicCost(
          'stripe',
          'payment_processing'
        );
        // Stripe: configured percentage + $0.30
        apiCost = (params.stripeAmount * stripePercentage) / 100 + 0.3;
      } else {
      ...
      if (error) {
        ...
        console.log(
          `[ApiCostTracker] Logged ${params.provider}/${params.operation}:`,
          {
            ...
            margin:
              creditValue > 0
                ? `${(((creditValue - apiCost) / creditValue) * 100).toFixed(1)}%`
                : 'N/A',
          }
        );
      }
   ...
   */
  private static calculateCreditValue(
    credits: number,
    userPlan: string
  ): number {
    ...
    switch (userPlan) {
      case 'basic':
        ...
        perCreditValue =
          this.CREDIT_PRICING.basic.price / this.CREDIT_PRICING.basic.credits;
        break;
      case 'starter':
        // Starter plan: $24.99 for 60 credits
        perCreditValue =
          this.CREDIT_PRICING.starter.price /
          this.CREDIT_PRICING.starter.credits;
        break;
      case 'professional':
      case 'pro':
        // Professional plan: $49.99 for 60 credits
        perCreditValue =
          this.CREDIT_PRICING.professional.price /
          this.CREDIT_PRICING.professional.credits;
        break;
      default:
        // Free or pay-as-you-go: use best matching package
        if (credits >= 50) {
          perCreditValue =
            this.CREDIT_PRICING.payg_50.price /
            this.CREDIT_PRICING.payg_50.credits;
        } else if (credits >= 20) {
          perCreditValue =
            this.CREDIT_PRICING.payg_20.price /
            this.CREDIT_PRICING.payg_20.credits;
        } else {
          perCreditValue =
            this.CREDIT_PRICING.payg_10.price /
            this.CREDIT_PRICING.payg_10.credits;
        }
    }
  ...
  static async getCostAnalytics(days: number = 30) {
    try {
      ...
      logs?.forEach(log => {
        ...
        byProvider[provider].totalProfit +=
          (log.credit_value || 0) - (log.api_cost || 0);
        ...
        byProvider[provider].operations[log.operation].cost +=
          log.api_cost || 0;
        byProvider[provider].operations[log.operation].revenue +=
          log.credit_value || 0;
