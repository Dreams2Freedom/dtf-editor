...
export class DeepImageService {
  ...
  public async upscaleImage(
    ...
    if (!this.apiKey) {
      return {
        status: 'error',
        error: 'Deep-Image.ai API key is not configured',
      };
    }
    ...
    try {
      ...
      const isExtremeDimensions =
        options.targetWidth &&
        options.targetHeight &&
        (options.targetWidth > 6000 ||
          options.targetHeight > 6000 ||
      ...
      if (imageUrl.startsWith('data:')) {
        console.log(
          '[DeepImage] Received data URL, need to convert to HTTP URL first'
        );
        ...
        try {
          ...
          if (uploadResult.success && uploadResult.url) {
            ...
            console.log(
              '[DeepImage] Successfully uploaded data URL to storage:',
              finalImageUrl
            );
          } else {
            return {
              status: 'error',
              error:
                'Failed to upload image to storage. Deep-Image requires HTTP URLs, not data URLs.',
            };
          }
        } catch (uploadError) {
          ...
          return {
            status: 'error',
            error:
              'Failed to process image data. Please try uploading the image file directly.',
          };
        }
      }
      ...
      if (options.targetWidth && options.targetHeight) {
        ...
        if (options.targetWidth < 100 || options.targetHeight < 100) {
          console.warn(
            '[DeepImage] Target dimensions too small, using 2x scale instead'
          );
        ...
        requestBody: {
          ...
          url:
            typeof requestBody.url === 'string'
              ? requestBody.url.substring(0, 100) + '...'
              : requestBody.url, // Truncate long URLs for logging
        },
      });

      // Make the API request with extended timeout for large images
      const controller = new AbortController();
      const timeoutMs = this.getTimeoutForDimensions(
        options.targetWidth,
        options.targetHeight
      );
      ...
      console.log(
        `[DeepImage] Using timeout of ${timeoutMs / 1000}s for dimensions ${options.targetWidth}x${options.targetHeight}`
      );
      ...
      try {
        ...
        return {
          status: 'error',
          error: 'Invalid response from Deep-Image.ai API',
        };
      }
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        return {
          status: 'error',
          error:
            'Processing timeout. The image is too large for real-time processing. Please try: 1) Using smaller dimensions, 2) Disabling AI enhancements (use basic upscale), or 3) Processing in smaller steps.',
        };
      }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to connect to Deep-Image.ai API',
      };
    }
  }
   ...
   */
  private async multiStepUpscale(
    imageUrl: string,
    options: UpscaleOptions
  ): Promise<UpscaleResponse> {
    ...
    console.log(
      '[DeepImage] Starting multi-step upscale to',
      targetWidth,
      'x',
      targetHeight
    );
    ...
    const step1Result = await this.upscaleImageWithoutCheck(
      imageUrl,
      step1Options
    );
   ...
   */
  private async upscaleImageWithoutCheck(
    imageUrl: string,
    options: UpscaleOptions
  ): Promise<UpscaleResponse> {
    if (!this.apiKey) {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          'Deep-Image.ai API error in multi-step:',
          response.status
        );
        return {
          status: 'error',
      ...
      return {
        status: 'error',
        error:
          error instanceof Error ? error.message : 'Multi-step upscale failed',
      };
    }
  }
      ...
      try {
        ...
        // Job failed
          ...
          return {
            status: 'error',
            error:
              result.error || result.message || 'Job failed during processing',
          };
        }
