...
export function calculateDPI(input: DPICalculationInput): DPICalculationResult {
  ...
  if (averageDPI >= 300) {
    ...
    recommendation = 'Perfect for professional DTF printing! Your image will produce crisp, high-quality transfers.';
  } else if (averageDPI >= 200) {
    qualityLevel = 'good';
    recommendation = 'Good quality for most DTF applications. Consider upscaling for best results on detailed designs.';
  } else if (averageDPI >= 150) {
    qualityLevel = 'fair';
    recommendation = 'Acceptable for simple designs but may appear pixelated. We recommend upscaling your image.';
  } else {
    qualityLevel = 'poor';
    recommendation = 'Too low for quality DTF printing. Your image needs upscaling to avoid pixelated results.';
  }
 ...
 */
export function getQualityColor(qualityLevel: DPICalculationResult['qualityLevel']): string {
  switch (qualityLevel) {
    case 'excellent':
      return 'green';
    case 'good':
 ...
 */
export function calculateRequiredPixels(printWidth: number, printHeight: number, targetDPI: number = 300): {
  width: number;
  height: number;
  totalPixels: number;
} {
