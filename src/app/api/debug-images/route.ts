...
export async function GET() {
  try {
    ...
    const { data: images, error: rpcError } = await serviceClient.rpc('get_user_images', {
      p_user_id: user.id
    });
    ...
    if (images && Array.isArray(images)) {
      for (const img of images.slice(0, 3)) { // Check first 3 images
        const urlCheck = {
          ...
          url_type: img.storage_url?.startsWith('https://') ? 'https' : 
                    img.storage_url?.startsWith('data:') ? 'data' : 'unknown',
