...
async function handlePost(request: NextRequest) {
  console.log('[WELCOME EMAIL API] Step 1: Request received at', new Date().toISOString());
  ...
  try {
    ...
    try {
      ...
      console.log('[WELCOME EMAIL API] Step 2: Body parsed:', JSON.stringify(body));
    } catch (e) {
    ...
    if (body.userId && body.email) {
      console.log('[WELCOME EMAIL API] Step 3: Using provided userId and email from body');
      userId = body.userId;
      userEmail = body.email;
    } else {
      console.log('[WELCOME EMAIL API] Step 3: No userId/email in body, checking auth');
    ...
    console.log('[WELCOME EMAIL API] Step 7: SUCCESS - Returning success response');
    return NextResponse.json({
      success: true,
