...
async function handleGetDatabaseStats(request: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      supabase.from('processed_images').select('*', { count: 'exact', head: true }),
    ...
    const [
      ...
      supabase.from('processed_images')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', oneDayAgo),
      supabase.from('users')
        .select('*', { count: 'exact', head: true })
    ...
    if (storageData) {
      for (const bucket of storageData) {
        ...
        if (files) {
          totalStorageSize += files.reduce((acc, file) => acc + (file.metadata?.size || 0), 0);
        }
      }
    }
    ...
    // Calculate totals
    stats.summary.total_records = Object.values(stats.tables)
      .reduce((acc, table) => acc + (typeof table === 'object' && 'total' in table ? table.total : 0), 0);
    
    // Calculate growth rate
    const newRecords24h = (recentUsersResult.count || 0) + (recentImagesResult.count || 0);
    if (stats.summary.total_records > 0) {
      stats.summary.growth_rate_24h = (newRecords24h / stats.summary.total_records) * 100;
    }
