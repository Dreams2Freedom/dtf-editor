...
async function handleGet(request: NextRequest) {
  try {
    ...
    const { data: allUsers, error: usersError } = await serviceClient
      .from('profiles')
      .select(
        'id, created_at, subscription_plan, subscription_status, last_activity_at, stripe_subscription_id, email'
      );
    ...
    const currentPeriodNewUsers = users.filter(
    ...
    const activeUsersThisPeriod = users.filter(
      u => u.last_activity_at && new Date(u.last_activity_at) >= startDate
    ).length;
    ...
    const activeUsersPreviousPeriod = users.filter(u => {
      ...
      return (
        activityDate >= previousPeriodStart && activityDate < previousPeriodEnd
      );
    }).length;
    ...
    const currentSubscribers = users.filter(
      u =>
    ...
    const basicSubscribers = users.filter(
      u => u.subscription_plan === 'basic'
    ).length;
    const starterSubscribers = users.filter(
    ...
    const currentPeriodTransactions =
      transactions?.filter(t => new Date(t.created_at) >= startDate) || [];
    ...
    const churnRate =
      paidUsers > 0 ? (churnedCount / (paidUsers + churnedCount)) * 100 : 0;

    // Calculate churn by plan
    const basicChurn =
      basicSubscribers > 0
        ? ((cancellations?.filter(c => c.event_data?.plan === 'basic').length ||
            0) /
            basicSubscribers) *
          100
        : 0;
    ...
    const starterChurn =
      starterSubscribers > 0
        ? ((cancellations?.filter(c => c.event_data?.plan === 'starter')
            .length || 0) /
            starterSubscribers) *
          100
        : 0;
    ...
    const conversionRate =
      totalUserBase > 0 ? (paidUsers / totalUserBase) * 100 : 0;
    ...
    const previousConversionRate =
      previousPeriodNewUsers > 0
        ? (previousPeriodPaidUsers / previousPeriodNewUsers) * 100
        : 0;
    const currentConversionRate =
      currentPeriodNewUsers > 0
        ? (currentPeriodPaidUsers / currentPeriodNewUsers) * 100
        : 0;
    ...
    const freeToBasicRate =
      totalUserBase > 0 ? (basicSubscribers / totalUserBase) * 100 : 0;
    const freeToStarterRate =
      totalUserBase > 0 ? (starterSubscribers / totalUserBase) * 100 : 0;
    ...
    const userGrowthRate =
      previousPeriodNewUsers > 0
        ? ((currentPeriodNewUsers - previousPeriodNewUsers) /
    ...
    const mauGrowth =
      activeUsersPreviousPeriod > 0
        ? ((activeUsersThisPeriod - activeUsersPreviousPeriod) /
    ...
    const day90Cohort = users.filter(u => {
      ...
      return (
        u.last_activity_at && new Date(u.last_activity_at) >= threeMonthsAfter
      );
    });
    ...
    const churnTrend =
      churnRate > 5 ? -Math.abs(churnRate - 5) : Math.abs(5 - churnRate);
