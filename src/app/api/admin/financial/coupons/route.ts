...
async function handleGet(request: NextRequest) {
  try {
    ...
    const metrics = {
      ...
      active_coupons: coupons?.filter(c => c.is_active && new Date(c.valid_until) >= now).length || 0,
      expired_coupons: coupons?.filter(c => new Date(c.valid_until) < now).length || 0,
      total_uses: coupons?.reduce((sum, c) => sum + (c.uses_count || 0), 0) || 0,
