...
async function handlePost(request: NextRequest) {
  ...
  try {
    ...
    if (targetWidth && targetHeight) {
      ...
      console.log('[Upscale] Using target dimensions:', {
        targetWidth,
        targetHeight,
      });
    } else if (scale) {
      ...
      console.error(
        '[Upscale] No scale or dimensions provided, defaulting to 2x'
      );
    ...
    if (result.success) {
            ...
            if (imageData?.storage_url) {
              ...
              if (urlData?.publicUrl) {
                ...
                if (!finalUrl.startsWith('http')) {
                  console.error(
                    '[Upscale] Generated URL is invalid:',
                    finalUrl
                  );
                ...
                finalUrl = `${env.SUPABASE_URL}/storage/v1/object/public/images/${imageData.storage_url}`;
                console.log(
                  '[Upscale] Manually constructed fallback URL:',
                  finalUrl
                );
              }
            } else {
      ...
      if (finalUrl?.startsWith('data:')) {
        console.error(
          '[Upscale] WARNING: Still have data URL after processing, this will cause navigation issues!'
        );
        return NextResponse.json(
          {
      ...
      if (finalUrl?.includes('deep-image.ai')) {
        console.warn(
          '[Upscale] WARNING: Returning Deep-Image URL. This URL will expire soon:',
    ...
    if (errorMessage.includes('timeout')) {
      return NextResponse.json(
        {
          error:
            'The image processing is taking longer than expected. Please try again with a smaller image or simpler processing options.',
