...
export default function TestClippingMagic() {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">ClippingMagic White Label Editor Test</h1>
      ...
      <div className="space-y-2 mb-6 p-4 bg-gray-50 rounded">
        ...
        <p>Initialized: <span className={initialized ? 'text-green-600' : 'text-red-600'}>{initialized ? 'Yes' : 'No'}</span></p>
        <p>API ID: {env.CLIPPINGMAGIC_API_KEY}</p>
        {uploadedImage && (
          <p className="text-green-600">Image uploaded: ID {uploadedImage.id}</p>
        )}
      </div>
      ...
      <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded">
        ...
        <ul className="list-disc list-inside text-sm space-y-1 text-yellow-700">
          <li>The editor opens in a <strong>popup window</strong>, not an iframe</li>
      ...
      <div className="mt-4 p-4 bg-gray-100 rounded">
        ...
        <p className="text-xs text-gray-600">Open your browser's developer console to see detailed logs</p>
      </div>
    </div>
  );
}