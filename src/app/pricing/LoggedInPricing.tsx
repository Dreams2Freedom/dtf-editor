 ...
 * subscription/pay-as-you-go checkout flow unchanged. Public (logged-out)
 * visitors are served the redesigned PublicPricingDetails page instead;
 * see client.tsx for the auth switch.
 */
export function LoggedInPricing() {
                ...
                <tbody>
                  {COMPARISON_FEATURES.map((feature, i) => (
                    ...
                    >
                      ...
                      {(
                        ['free', 'basic', 'starter', 'professional'] as const
                      ).map(plan => {
                        const val = feature[plan];
                        return (
                          <td
                            key={plan}
                            className={`text-center py-3 px-4 ${plan === 'basic' ? 'bg-amber-50/30' : ''}`}
                          >
                            {val === true ? (
