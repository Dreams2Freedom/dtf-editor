...
export default function BackgroundRemovalClient() {
  ...
  useEffect(() => {
    const fetchImageFromUrl = async (url: string) => {
      try {
        ...
        if (url.startsWith('data:')) {
          ...
          console.log(
            '[Background Removal] Created file from data URL:',
            file.size,
            'bytes',
            file.type
          );
          ...
          console.log(
            '[Background Removal] Created file from URL:',
            file.size,
            'bytes',
            file.type
          );
    ...
    if (tempImageKey) {
      try {
        ...
        if (storedImage) {
          console.log(
            '[Background Removal] Retrieved image from sessionStorage'
          );
        ...
        console.error(
          '[Background Removal] Failed to retrieve from sessionStorage:',
          err
        );
      }
    }
  ...
  const compressImage = async (
    ...
    if (file.size <= maxSizeBytes) {
      console.log(
        '[Background Removal] Image under size limit, NO compression needed:',
        (file.size / 1024 / 1024).toFixed(2),
        'MB'
      );
      console.log(
        '[DEBUG] Returning original file without compression - preserving full quality'
      );
      return file;
    }

    console.log(
      '[Background Removal] File too large, need to compress:',
      (file.size / 1024 / 1024).toFixed(2),
      'MB'
    );
      ...
      reader.onload = e => {
        ...
        img.onload = () => {
          ...
          if (width > maxDimension || height > maxDimension) {
            ...
            console.log(
              '[Background Removal] Scaling down from',
              img.width,
              'x',
              img.height,
              'to',
            ...
            console.log(
              '[Background Removal] Preserving original dimensions:',
              width,
              'x',
              height
            );
          ...
          if (file.type === 'image/png' && file.size > maxSizeBytes) {
            ...
            console.log(
              '[Background Removal] Converting PNG to JPEG for better compression'
            );
          }
              ...
              blob => {
                if (blob) {
                  if (blob.size > maxSizeBytes && quality > 0.5) {
                    ...
                    console.log(
                      '[Background Removal] Compressed to:',
                  ...
                  console.log(
                    '[Background Removal] Failed to compress, using original'
                  );
      ...
      <main className="py-8">
        <div className="max-w-4xl mx-auto p-6">
          ...
          <div className="mt-6 flex justify-center">
            <NeedHelpButton toolKey="background-removal" />
          </div>
        </div>
      </main>
