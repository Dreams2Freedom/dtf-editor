  ...
  const detectImageDimensions = useCallback(
    async (url: string) => {
      try {
        ...
        console.log(
          `Default print size set to ${defaultWidth.toFixed(2)}" × ${defaultHeight.toFixed(2)}"`
        );
        console.log(
          `Image quality at this size: ${Math.round(actualDPI)} DPI (300 DPI recommended for print)`
        );
      } catch (error) {
  ...
  const calculateUpscaleFactor = useCallback(() => {
    ...
    if (requiredWidth > maxDimension || requiredHeight > maxDimension) {
      ...
      const limitScale = Math.min(
        maxDimension / imageDimensions.width,
        maxDimension / imageDimensions.height
      );
    ...
    return {
      ...
      requiredWidth,
      requiredHeight,
      currentDPI: Math.round(
        Math.min(imageDimensions.width / width, imageDimensions.height / height)
      ),
      limitedByMax,
    };
        ...
        const handleUpload = async () => {
          try {
            ...
            if (storedImage.startsWith('data:')) {
              ...
              const mimeType =
                storedImage.match(/data:([^;]+)/)?.[1] || 'image/jpeg';
            ...
            if (uploadData.success && uploadData.publicUrl) {
              ...
              setError(
                uploadData.error ||
                  'Failed to upload image. Please try uploading again.'
              );
            }
            setIsLoading(false);
          } catch (err) {
            ...
            setError(
              'Failed to process image. Please try uploading again from the form below.'
            );
            setIsLoading(false);
    ...
    try {
      ...
      try {
        ...
        if (await needsCompression(file, 10, 4096)) {
          console.log(
            '[Upscale] Large file detected, compressing before upload'
          );
          ...
          console.log(
            '[Upscale] Medium file detected, uploading file instead of URL'
          );
          fileToUpload = file;
          shouldUploadFile = true;
        }
        ...
        console.log(
          '[Upscale] Could not convert URL to file, using URL directly:',
          err
        );
      ...
      if (mode === 'dpi') {
        ...
        if (upscaleInfo.scale < 1.2) {
          // Less than 20% increase
          console.log(
            '[Upscale] Image already meets DPI requirements, using 2x scale instead'
          );
          ...
          upscaleInfo = null;
          setError(
            'Your image already meets the required DPI. Applying 2x upscale with enhancements instead.'
          );
          ...
          if (megapixels > 20) {
            ...
            console.warn(
              '[Upscale] Forcing basic mode for large image (>20MP):',
              megapixels.toFixed(2),
              'MP'
            );
            processingMode = 'basic_upscale';
            setError(
              'Using fast processing mode for large image. AI enhancements disabled.'
            );
            setTimeout(() => setError(null), 3000);
          }
          ...
          if (upscaleInfo.limitedByMax) {
            console.log(
              '[Upscale] Dimensions were limited to prevent timeout:',
              {
          ...
          if (actualMode === 'dpi' && upscaleInfo) {
            formData.append(
              'targetWidth',
              upscaleInfo.requiredWidth.toString()
            );
            formData.append(
              'targetHeight',
              upscaleInfo.requiredHeight.toString()
            );
          }
          ...
          if (upscaleInfo.limitedByMax) {
            ...
            setError(
              `Note: Image dimensions limited to prevent timeout. Result will be ${actualDPI} DPI instead of 300 DPI.`
            );
      ...
      if (response.status === 504) {
        throw new Error(
          'The server took too long to process your image. Please try again with a smaller image or simpler settings.'
        );
      }

      if (response.status === 502 || response.status === 503) {
        throw new Error(
          'The image processing service is temporarily unavailable. Please try again in a few moments.'
        );
      }
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        if (response.status === 504 || text.includes('Gateway Timeout')) {
          throw new Error(
            'The server took too long to process your image. Please try again with a smaller image.'
          );
        }
      ...
      if (!response.ok) {
        throw new Error(
          result.error || `Processing failed with status ${response.status}`
        );
      }
      ...
      if (err instanceof Error) {
        if (err.name === 'AbortError') {
          setError(
            'Processing is taking longer than expected. For large images, please try: 1) Reducing image dimensions before uploading, 2) Using a smaller upscale factor, or 3) Disabling AI enhancements.'
          );
        } else {
                  ...
                  <div ref={resultRef} className="space-y-4 scroll-mt-6">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-sm text-green-700">
                          Your image is ready — download it or send it to
                          another tool below. It&apos;s also saved to your
                          account.
                        </p>
                      </div>
                    </div>
