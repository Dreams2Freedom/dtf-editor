...
if (CONFIG.concurrentUsers > CONFIG.maxConcurrentUsers) {
  console.error(`🛑 SAFETY: Limiting concurrent users to ${CONFIG.maxConcurrentUsers} for production`);
  CONFIG.concurrentUsers = CONFIG.maxConcurrentUsers;
}

if (CONFIG.requestsPerUser > CONFIG.maxRequestsPerUser) {
  console.error(`🛑 SAFETY: Limiting requests per user to ${CONFIG.maxRequestsPerUser} for production`);
...
async function simulateUser(userId) {
  console.log(`${colors.cyan}User ${userId} started (with safety delays)${colors.reset}`);
...
function printResults() {
  ...
  console.log(`  Safety Delay: ${CONFIG.delayBetweenRequests}ms between requests`);
  console.log(`  Duration: ${duration.toFixed(2)} seconds`);
  
  console.log(`\n${colors.bright}Request Statistics:${colors.reset}`);
  console.log(`  Total Requests: ${totalRequests}`);
  console.log(`  ${colors.green}Successful: ${results.successful} (${((results.successful/totalRequests)*100).toFixed(1)}%)${colors.reset}`);
  console.log(`  ${colors.red}Failed: ${results.failed} (${((results.failed/totalRequests)*100).toFixed(1)}%)${colors.reset}`);
  ...
  Object.entries(endpointStats).forEach(([endpoint, times]) => {
    ...
    console.log(`  ${endpoint}: ${avg.toFixed(2)}ms (${times.length} requests)`);
  });
  ...
  if (stats.p95 < 2000) {
    ...
    console.log(`  ${colors.yellow}⚠ Acceptable: P95 < 5 seconds${colors.reset}`);
  } else {
  ...
  if (results.failed / totalRequests < 0.01) {
    ...
    console.log(`  ${colors.red}✗ High error rate (${((results.failed/totalRequests)*100).toFixed(1)}%)${colors.reset}`);
  }
...
async function runProductionTest() {
  console.log(`${colors.yellow}${colors.bright}⚠️  PRODUCTION LOAD TEST${colors.reset}`);
  console.log(`${colors.yellow}Testing with care to avoid overloading production${colors.reset}\n`);
  
  console.log(`Testing: ${CONFIG.baseUrl}`);
  console.log(`Users: ${CONFIG.concurrentUsers} (max: ${CONFIG.maxConcurrentUsers})`);
  console.log(`Requests per user: ${CONFIG.requestsPerUser} (max: ${CONFIG.maxRequestsPerUser})`);
  console.log(`Safety delay: ${CONFIG.delayBetweenRequests}ms between requests\n`);
  
  // Confirmation prompt for safety
  console.log(`${colors.yellow}Press Ctrl+C now to cancel, or wait 3 seconds to continue...${colors.reset}`);
  ...
  console.log(`${colors.yellow}Monitor your Vercel and Supabase dashboards for any issues.${colors.reset}`);
}
