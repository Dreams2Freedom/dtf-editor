...
async function checkKPIData() {
  ...
  if (transError) {
    ...
    if (transactions.length > 0) {
      ...
      transactions.slice(0, 3).forEach((t, i) => {
        ...
        console.log(`  - Price paid: $${t.metadata?.price_paid ? (t.metadata.price_paid / 100).toFixed(2) : 'N/A'}`);
  ...
  const freeUsersCount = users?.filter(u => 
    !u.subscription_plan || u.subscription_plan === 'free'
  ).length || 0;
  ...
  console.log(`- Conversion rate would be: ${totalUsers > 0 ? ((paidUsersCount / totalUsers) * 100).toFixed(2) : 0}%`);
  
  // Check for ARPU
  const basicCount = users?.filter(u => u.subscription_plan === 'basic').length || 0;
  const starterCount = users?.filter(u => u.subscription_plan === 'starter').length || 0;
  const proCount = users?.filter(u => u.subscription_plan === 'professional' || u.subscription_plan === 'pro').length || 0;
