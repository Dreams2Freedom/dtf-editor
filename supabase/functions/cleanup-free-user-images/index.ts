...
serve(async req => {
  ...
  try {
    ...
    console.log(
      `Found ${imagesToDelete?.length || 0} images to check for deletion`
    );
      ...
      try {
        ...
        if (profile.last_credit_purchase_at) {
          ...
          if (daysSincePurchase <= 90) {
            console.log(
              `Skipping image ${image.id} - user has recent credit purchase`
            );
            skippedCount++;
            continue;
          }
        ...
        if (image.storage_url) {
          ...
          const urlParts = image.storage_url.split(
          ...
          if (urlParts.length > 1) {
            ...
            if (storageError) {
              console.error(
                `Error deleting storage for ${image.id}:`,
                storageError
              );
        ...
        if (image.thumbnail_url) {
          const urlParts = image.thumbnail_url.split(
            '/storage/v1/object/public/'
          );
          if (urlParts.length > 1) {
            const [bucket, ...pathParts] = urlParts[1].split('/');
            ...
            if (thumbnailError) {
              console.error(
                `Error deleting thumbnail for ${image.id}:`,
                thumbnailError
              );
            }
          }
        ...
        if (dbError) {
          console.error(
            `Error deleting database record for ${image.id}:`,
            dbError
          );
