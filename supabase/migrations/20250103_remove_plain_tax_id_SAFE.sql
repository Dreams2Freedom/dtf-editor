-- Migration: Remove plain text tax_id field to enforce encryption (SAFE VERSION)
-- Date: January 2025
-- Purpose: Security hardening - ensure tax IDs are only stored encrypted
-- This version checks for existing constraints to avoid conflicts
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'affiliates'
    AND constraint_name = 'check_tax_id_encrypted_format'
  ) THEN
    ALTER TABLE public.affiliates
    ADD CONSTRAINT check_tax_id_encrypted_format
    CHECK (
      tax_id_encrypted IS NULL
      OR LENGTH(tax_id_encrypted) >= 64
    );
    RAISE NOTICE 'Added check_tax_id_encrypted_format constraint';
  ELSE
    RAISE NOTICE 'check_tax_id_encrypted_format constraint already exists, skipping';
  END IF;
END $$;

-- Create an audit table for tax data access (for compliance) - only if it doesn't exist
...
-- Index for audit log queries (only if not exists)
CREATE INDEX IF NOT EXISTS idx_tax_audit_affiliate ON public.tax_data_audit_log(affiliate_id);
CREATE INDEX IF NOT EXISTS idx_tax_audit_user ON public.tax_data_audit_log(user_id);
CREATE INDEX IF NOT EXISTS idx_tax_audit_created ON public.tax_data_audit_log(created_at);

-- Enable RLS on audit log
ALTER TABLE public.tax_data_audit_log ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist and recreate
DROP POLICY IF EXISTS "Service role can insert audit logs" ON public.tax_data_audit_log;
CREATE POLICY "Service role can insert audit logs"
  ON public.tax_data_audit_log FOR INSERT
  TO service_role
  WITH CHECK (true);

DROP POLICY IF EXISTS "Service role can view audit logs" ON public.tax_data_audit_log;
CREATE POLICY "Service role can view audit logs"
  ON public.tax_data_audit_log FOR SELECT
  TO service_role
  USING (true);

-- Add function to validate encrypted data format (with replace)
...
-- Add trigger to validate encrypted data on insert/update (with replace)
